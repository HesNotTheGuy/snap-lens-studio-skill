...
- **Version compatibility** — Camera Kit pins Lens Studio versions to SDK versions ([compatibility matrix](https://developers.snap.com/camera-kit/ar-content/lens-studio-compatibility)); verified rows include **LS 5.18.x → Mobile 1.46.x / Web 1.14.x**, **LS 5.17.x → Web 1.13.x (Mobile n/a)**, LS 5.16.x → Mobile 1.45.x / Web 1.12.x, LS 5.13.x → Mobile 1.43.x / Web 1.9.x, down to LS 4.36.x. Older-LS Lenses keep running in newer Camera Kit. **This matrix drifts — re-fetch it at build time.**
- **Unsupported on Camera Kit** — Licensed Sounds, Remote Service-enabled Lenses, Scan, Spatial Persistence, VoiceML/Speech recognition/Text-to-speech, Ray Tracing (iOS/Mobile), and — Web-only — Face True Depth and Location AR. **Connected Lenses** ("Multi-User Services") is listed as **"requires inquiry" for Mobile and "unsupported" for Web** — don't rely on it without confirming with Snap. **Bitmoji** is **not mentioned** in the compatibility page at all; treat it as "not documented / assume unsupported" rather than an explicitly confirmed exclusion.
...
- **Camera Kit caveat** — Bitmoji is not listed as supported in the Camera Kit compatibility matrix; don't assume it works there.
...
- **Camera Kit** — pin LS to the published matrix (e.g., 5.18.x ↔ Mobile 1.46.x / Web 1.14.x); re-fetch at build time.
