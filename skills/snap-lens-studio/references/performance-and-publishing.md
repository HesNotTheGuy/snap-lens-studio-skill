...
**Other caps:** Lens **name ≤ 18 characters** (letters/numbers/spaces only — [project-info](https://developers.snap.com/lens-studio/publishing/configuring/configuring-project-info)); **≤ 10 liquify effects** per render order + face index (else crash); recommended **one ML Component** per Lens. *(320×320 is **not** stated as an icon requirement on the project-info or submission pages, but it is Lens Studio's own default icon size — the fallback asset is `lens_default_icon_320.png` — and the MCP icon recipe in `fundamentals-and-workflow.md` is verified at 320×320. Author at 320×320 or a larger square; see that file for the full-bleed rules.)*
...
The [Submission Guidelines](https://developers.snap.com/lens-studio/publishing/submitting/submission-guidelines) reject: sexual content, harassment (including auto-sorting friends by relationship strength), violence, weapons, misinformation, illegal/regulated goods, hate, and creative-quality failures — including **growth hacks** ("Follow for more", "Share with 10 Friends" — named explicitly on the page), prohibited QR codes, IP/trademark violations, and ChatGPT-integration limits. Re-verified 2026-09-07.
...
- **Lens Creator Rewards has two programmes, and a Lens joins exactly one — permanently** ([Lens Creator Rewards](https://developers.snap.com/lens-studio/monetization/lens-creator-rewards), re-verified 2026-09-07). **Top Performer Payouts** are measured in **unique users who post a Snap using your Lens** (in Qualified Regions), counted **within the first 90 days after submission** — a one-shot window, not a rolling one; Snap publishes no numeric threshold and the bar moves, so check the page rather than trusting a number you read somewhere. **Lens+ Payouts** (introduced 2025-10-16; application approval required) let a creator mark a Lens as exclusive to Lens+ / Platinum subscribers for a revenue share based on the engagement it receives. **Once enrolled, a Lens cannot switch programmes or unenrol — it can only be deleted.** The programme choice at submission is therefore a design decision: breadth of reach for Top Performer (a Lens that launches slowly has spent its window), a Lens people keep coming back to for Lens+.
- **Lens types Snap names as ineligible for rewards:** basic filters, beautification tools, static 2D images, frames, and copies of existing work. Design consequence: a Lens whose whole effect is Face Retouch / smoothing / whitening cannot earn regardless of reach, and a straight re-skin of an existing Lens will not either. Other gates on the same page: creator 18+, a 42-country qualifying-region list, and rewards becoming available roughly 14 days after the notification.
