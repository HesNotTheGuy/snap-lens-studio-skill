...
**Current version: Lens Studio 5.24.0** (released 2026-09-10; verified on ar.snap.com 2026-09-11). Headline **5.24**: **Connected Framework** (a multiplayer matchmaking SDK on top of Connected Lenses) with Multiplayer Starter and Game templates and a **Game Manager** state machine; **Game Suite 2.0**; a **Physics 2D** starter template and package (Bitmoji ragdolls, vehicles, pinball); **AI Video Transform** and **AI Photo Duo** GenAI plugins; **SPECS Account** sign-in for SPECS 27 projects (SPECS and Snapchat accounts stay signed in side by side); **Custom 2D Text materials**. ⚠️ **Change that touches existing lenses: the default blend mode for 2D Text moved from PremultipliedAlpha to Normal.** Opening a project in 5.24 rewrites text components set to PremultipliedAlpha to Normal automatically; text on any *other* blend mode is left alone and Snap says it "may experience visual changes" — re-check every lens with text after the project update. Editor API: the callback `fetch(request, onSuccess, onFailure)` on `AssetListService` / `MusicListService` is removed (it could crash on shutdown); use `fetchAsync(request)` (to be renamed `fetch` later). Known issue: Leaderboard lenses published from 5.24 need Snapchat 14.23 (they error on 14.21 and 14.22). Intel Macs: 5.25 is the last Intel build; 5.26+ requires Apple silicon. Glasses: the 5.24.0 download page's Spectacles line reads "The current latest Spectacles Firmware does not support this Lens Studio version" (2026-09-11), and the notes say the SPECS content is for SPECS 27, not Spectacles (2024).

**Measured: opening a 5.23.2 project in 5.24.0 (2026-09-11).** `model.openProject()` over MCP migrated it with no dialog. What it wrote: the `.esproj` (`studioVersion` → 5.24, **`iconHash` preserved**), a `//@component` line prepended to the legacy JS script (keep it in your source copy, or a rebuild from source strips it again), and every package in `Packages/` re-stamped (`package.native`: minor/patch/build, `UpdateCheckpoint` 102 → 107, `coreVersion` 372 → 377, `clientVersion` 14.16 → 14.21). Untouched: `Scene.scene`, the graph shader, the icon files. It also wrote its own `BackUp/5.23.2.26081320.zip` (83 entries) inside the project. The lens, its first-run tour, its taps and its persistent-store restore behaved identically afterwards. The build config reports **Minimum Client Version 14.21.0**, so lenses published from 5.24 need Snapchat 14.21 or newer. Zip the project yourself before opening it in a new version regardless; a lens folder is rarely under version control.
...
- **5.24 (2026-09-11): the MCP link survives an app update and restart** (the bearer token is keychain-backed) - the
  first `RunAndCollectLogsTool` call after the update already reported the new log file. **`ExecuteEditorCode` now
